...
function Ball(x, y, diameter, color, xSpeed, ySpeed){
	...
	this.color = color || [0, 0, 0];													// || [0, 0, 0] makes the default color black
...
function draw() {
	...
	for(var i = 0; i < ballList.length; i++){
		...
		if(ballList[i].xCoor >= canvas.width){											//if hit the right/left edge of the canvas
		...
		if(ballList[i].yCoor >= canvas.height){											//makes the circle diameter random
